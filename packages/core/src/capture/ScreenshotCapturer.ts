...
import { type PrivacyOptions, type ResolvedPrivacy, resolvePrivacy } from "../privacy";
...
/**
 * Screenshot-relevant subset of {@link PrivacyOptions}.
 *
 * @deprecated Use {@link PrivacyOptions}. Retained so existing imports keep
 * compiling; it is structurally assignable to the replacement.
 */
export type ScreenshotPrivacyOptions = PrivacyOptions;

/** Blur radius applied to masked elements. Wide enough to defeat upscaling. */
const MASK_BLUR_PX = 12;

/** Opaque fill drawn over blocked elements. */
const BLOCK_FILL_COLOR = "#6b7280";

/**
 * Blur masked elements and overwrite blocked ones in the cloned document.
 *
 * Runs against html2canvas's clone, never the live DOM, so the user's page is
 * untouched. An invalid selector is skipped rather than thrown: a typo in one
 * entry must not disable masking for every other entry.
 */
function applyPrivacyToClone(doc: Document, privacy: ResolvedPrivacy): void {
  for (const selector of privacy.maskSelectors) {
    for (const el of queryAllSafe(doc, selector)) {
      el.style.setProperty("filter", `blur(${MASK_BLUR_PX}px)`, "important");
    }
  }

  for (const selector of privacy.blockSelectors) {
    for (const el of queryAllSafe(doc, selector)) {
      el.style.setProperty("background", BLOCK_FILL_COLOR, "important");
...
/** `querySelectorAll` that treats an invalid selector as matching nothing. */
function queryAllSafe(doc: Document, selector: string): HTMLElement[] {
  try {
...
export class ScreenshotCapturer {
  ...
  /**
   * @param privacy - Options merged over the built-in defaults. Passing nothing
   * still masks passwords, one-time codes, and payment fields.
   */
  constructor(privacy: PrivacyOptions = {}) {
    this.privacy = resolvePrivacy(privacy);
  }
